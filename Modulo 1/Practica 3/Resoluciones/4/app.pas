...
procedure buscar(a: arbol; d: integer; var out: arbol);
begin
    out := Nil;
    if ( a <> Nil ) then begin
        if ( d = a^.dato ) then begin
            out := a;
        end
        else if ( d > a^.dato ) then begin
            buscar(a^.HD, d, out);
        end
        else begin
            buscar(a^.HI, d, out);
...
begin
    ...
    imprimirpornivel(a);
    Write('Introducir el valor a buscar:');
    readln(valorABuscar);
    buscar(a, valorABuscar, salida);
    if ( salida <> Nil ) then
        Writeln('Valor encontrado: ', salida^.dato)
    else
        writeln('Valor encontrado: Nil');
    readln;
end.
