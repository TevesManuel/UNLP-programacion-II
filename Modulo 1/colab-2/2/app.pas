...
type
    ...
    arbolPacientes = ^nodoAPaciente;
    nodoAPaciente = record
        dni: LongInt;
        HD: arbolPacientes;
        HI: arbolPacientes;
    end;
...
procedure agregarAABP(var a: arbolPacientes; d: LongInt);
var
    nue: arbolPacientes;
begin
    if ( a = Nil ) then begin
        ...
        if (d > a^.dni) then begin
            agregarAABP(a^.HD, d);
        end
        else if ( d < a^.dni) then begin
            agregarAABP(a^.HD, d);
        end;
    end;
end; 
...
begin
    if ( a = Nil ) then begin
        ...
        nue^.cp := d.cp;
        nue^.ciudad := d.ciudad;
        agregarAABP(nue^.pacientes, d.dni);
        nue^.HI := Nil;
        nue^.HD := Nil;
        a := nue;
    end
...
function contarAABP(a: arbolPacientes): integer;
begin
    if ( a <> Nil ) then begin
        contarAABP := contarAABP(a^.HI) + contarAABP(a^.HD) + 1;
    end else begin
        contarAABP := 0;
    end;
end;
    ...
    if ( a <> Nil) then begin
        if ( a^.cp > min ) then begin
            if ( a^.cp < max ) then begin
                out := out + contarAABP(a^.pacientes);
