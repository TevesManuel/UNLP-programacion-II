...
begin
    ...
    else begin {no tenemos el árbol vacío}
...
procedure BusquedaApellido(a: arbol; apellido: string);
begin
    if ( a <> nil ) then begin
        BusquedaApellido(a^.HI, apellido);
        if a^.dato.apellido = apellido then
            ImprimirDato(a^.dato);
        BusquedaApellido(a^.HD, apellido)
    end;
end;
...
begin
    ...
    {Recorridos}
    writeln('Ingrese un DNI'); read(dni);
    ...
    WriteLn('Ingrese un apellido: '); Readln(apellido);
    BusquedaApellido(arbolCurso, apellido);
    readln()
end.